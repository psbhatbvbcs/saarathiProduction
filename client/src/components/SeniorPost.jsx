...
const SeniorPost = () => {
    ...
    return (
        <StyledDashboard>
            <Box sx={{ minHeight: "90vh", bgcolor: mode === "light" ? "wheat" : "" }} m={isNonMobile ? "0rem 3rem" : "1rem 0"} borderRadius={"30px"} display={"flex"} flexDirection={"column"} alignItems={"center"} >

                <Title>{postData.title}</Title>


                <Box width={"70%"} display={"flex"} alignItems={"center"} justifyContent={"flex-end"} gap={"15px"}>
                    <Typography variant='body1' fontWeight={600} justifyContent={'flex-end'}>-{postData.author}</Typography>
                    <Avatar src={`${server}/public${postData.filePath}`} sx={{ width: "50px", height: "50px" }} />
                </Box>
                <Box width={"100%"} display={"flex"} justifyContent={"center"} m={"1rem"}>
                    <div
                        style={{
                            margin: "2rem",
